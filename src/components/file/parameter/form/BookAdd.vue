<template>
  <div>
    <span class="title_span">添加</span>
    <div style="padding:0 100px 30px 0;margin-top:30px;">
      <el-form :rules="rules_book_edit" ref="file" :model="formData" label-width="150px">
        <el-row :gutter="20">
          <el-col :span="12">
  ...
  methods: {
    ...
    saveBook() {
      ...
      this.$refs.file.validate(valid => {
        if (valid) {
          this.$store.dispatch("book/addBookData", this.formData);
        } else {
        }
      });
      // 编辑
